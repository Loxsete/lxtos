...
= NocturneOS Kernel API Documentation
...
- #link(<memory-management-pmm>)[ - Memory Management (PMM) ]

- #link(<virtual-memory-manager-vmm>)[ - Virtual Memory Manager (VMM) ]

- #link(<heap-allocator-kmalloc>)[ - Heap Allocator (kmalloc) ]

- #link(<virtual-filesystem-vfs>)[ - Virtual Filesystem (VFS) ]
...
- #link(<initramfs>)[ - InitRAMFS ]

- #link(<kernel-print>)[ - Kernel Print ]

- #link(<string-library>)[ - String Library ]

- #link(<kernel-api-table-kapi>)[ - Kernel API Table (kapi) ]

- #link(<syscall-dispatch>)[ - Syscall Dispatch ]

- #link(<architecture-gdt>)[ - Architecture: GDT ]

- #link(<architecture-idt>)[ - Architecture: IDT ]

- #link(<architecture-tss>)[ - Architecture: TSS ]
...
`include/mm/pmm.h` | `src/mm/pmm.c`


Bitmap-based physical page frame allocator. The bitmap is statically allocated (128 KiB, enough for 1M pages). Limine's memmap is used to figure out which regions are actually usable.
...
Starts with all pages marked as used, then walks the Limine memory map and frees usable regions. Also accumulates total RAM across usable + bootloader + kernel sections for reporting. Prints free/total to the framebuffer on boot.
...
Linear scan through the bitmap for the first free page. Marks it used and returns the physical address. Returns `0` on failure — callers should check this.
...
Same idea as `pmm_alloc` but finds `n` _contiguous_ free pages. Returns the base physical address of the region, or `0` if nothing fits.
...
`include/mm/vmm.h` | `src/mm/vmm.c`


4-level page table manager for x86-64. Physical↔virtual translation goes through Limine's HHDM offset. Intermediate tables are allocated on demand via the PMM.
...
Builds the initial kernel page table from scratch. Maps the first 4 GiB through HHDM and the kernel image at its virtual base. Switches to the new PML4 before returning.
...
Maps one 4 KiB page `virt` → `phys`. Walks (and creates if needed) PDPT → PD → PT, then writes the final PTE. Common flag combinations:
...
Allocates a fresh PML4 and copies the upper half (indices 256–511) from the current page table. This gives a new address space that still shares the kernel mappings.
...
`include/lib/kmalloc.h` | `src/lib/kmalloc.c`


A basic free-list allocator. Each block has an inline header storing its size and free flag. Adjacent free blocks get coalesced on every `kfree` call.
...
Sets up the heap at `start`. Called once from `vfs_setup()` after the VMM is up, using PMM-allocated pages converted to virtual addresses.
...
Allocates `size` bytes, aligned to 8. Splits the chosen block if there's enough leftover space. Returns `NULL` if no free block fits.
...
`include/fs/vfs.h` | `src/fs/vfs.c`


A minimal VFS layer. Every node is a `vfs_node_t` with an ops pointer; actual work is delegated to the filesystem that owns the node. Mount points are tracked in a flat table.
...
Mounts `fs_root` at `path`. Mounting at `"/"` sets the global root. For everything else, `vfs_resolve` finds the target node and sets its `mount` pointer. Returns `0` on success, `-1` if the table is full (`VFS_MAX_MOUNTS`).
...
Walks the tree component by component, following mount points when it hits them. Returns `NULL` if any component doesn't exist.
...
`include/fs/tmpfs.h` | `src/fs/tmpfs.c`


In-memory filesystem. Files store their content in `kmalloc`'d buffers that grow on write. Directories keep a flat array of child pointers (max `TMPFS_MAX_CHILDREN = 64` per dir).
...
All directory ops are supported: `finddir`, `readdir`, `mkdir`, `mkfile`, `unlink`. File write automatically reallocates the buffer if the new size exceeds the current one.
...
`include/fs/procfs.h` | `src/fs/procfs.c`


Read-only virtual filesystem for exposing kernel state. Each file is backed by a callback that generates its content on read.
...
`include/fs/ext2.h` | `src/fs/ext2.c`
...
Reads the superblock, checks for magic `0xEF53`, builds an `ext2_fs_t` context, and returns a VFS node for inode 2 (root). Returns `NULL` if the disk isn't ext2.
...
`include/fs/initramfs.h` | `src/fs/initramfs.c`
...
Walks the CPIO archive and creates VFS nodes under `root`. Handles both `070701` and `070702` magic. Creates parent directories as needed, skips `.` and empty entries, stops at `TRAILER!!!`. Always returns `0`.


The archive gets linked into the kernel via `ld -r -b binary` and is referenced through `_binary_build_initramfs_cpio_start` / `_binary_build_initramfs_cpio_end`.
...
`include/console/kprint.h` | `src/console/kprint.c`
...
`include/lib/kstring.h` | `src/lib/kstring.c`
...
`include/lib/kitoa.h` | `src/lib/kitoa.c`
...
#table(
  ...
  "kitoa_hex", "void kitoa_hex(uint64_t val, char *buf)", "uint64_t → 16-char uppercase hex, no 0x prefix; buf must be ≥ 17 bytes",
)
...
`include/kernel/kapi.h` | `src/kernel/kapi.c`


A structure of function pointers that is passed to ELF functions in userspace by the ELF loader. Instead of going through system calls for everything, userspace can call kernel procedures directly through this table. The structure is versioned (the `version` field) so userspace can check for compatibility. Not yet used
...
`include/kernel/syscall.h` | `src/kernel/syscall.c`


Triggered by `int 0x80` from userspace (vector 128). The ISR saves registers and calls `syscall_dispatch` with the syscall number from `rax` and arguments from `rdi`, `rsi`, `rdx`.
...
After `lgdt`, segment registers are reloaded via a far return trick in `gdt_flush_segments`, then `tss_load(0x28)` loads the TSS selector.
...
Writes a 64-bit gate entry for vector `vec`. `flags` = type + DPL byte, e.g. `0x8E` for a kernel interrupt gate, `0xEE` for a user-accessible one.
...
Allocates a 16 KiB kernel stack via `pmm_alloc_n(4)`, converts it to a virtual address, and writes the stack top into `kernel_tss.rsp0`. This is the stack the CPU switches to on any ring-3 → ring-0 transition (syscalls, exceptions). Must be called after both the PMM and GDT are initialized.
